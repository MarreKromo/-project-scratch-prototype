...
goto('saved');
 ...
 {screen==='evidence'&&<Page><Eyebrow>Why this focus?</Eyebrow><h1>Evidence, not a story.</h1>{recapMetrics?<Card><Stat a="Penalty strokes" b={recapMetrics.penalties}/><Stat a="Right misses" b={recapMetrics.right}/><Stat a="Three-putts" b={recapMetrics.threePutts}/><Stat a="GIR" b={`${recapMetrics.girPct}%`}/></Card>:<Notice>Complete a round to build evidence.</Notice>}<Card><b>Coach interpretation</b><p>{recapAnalysis.reason}</p><small>Confidence: {recapAnalysis.confidence}. This prototype uses deterministic rules, not an LLM, to keep the evidence traceable.</small></Card><Primary onClick={()=>goto('focus')}>Continue</Primary></Page>}
